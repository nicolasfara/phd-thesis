# Engineering Collective Systems in the Wearable Edge-Cloud Continuum: Models and Platform

## Papers

As following, the list of papers relevant to the thesis divided in hypothetical chapters.

### Languages

- **A Language-based Approach to Macroprogramming for IoT Systems through Large Language Models**
  - _Authors_: Gianluca Aguzzi, Nicolas Farabegoli, Mirko Viroli
  - _DOI_: [10.1145/3758326](https://doi.org/10.1145/3758326)
  - _Year_: 2025

- **Capabilities to Catch 'em All: Unify Choreography, Multitier and Aggregate Computing**
  - _Authors_: Nicolas Farabegoli, Pascal Weisemburger, Mirko Viroli, Guido Salvaneschi
  - _DOI_: Currently under review at [ACM Transactions on Programming Languages and Systems](https://dl.acm.org/journal/toplas).
  - _Year_: 2026

- **ScalaTropy: Multiparty Coordination with Monadic Communication Primitives**
  - _Authors_: Nicolas Farabegoli, Luca Tassinari, Gianluca Aguzzi, Mirko Viroli
  - _DOI_: Accepted at [COORDINATION 2026](http://www.discotec.org/2026/coordination.html).
  - _Year_: 2026
  
### Platforms/Deployments

- **Flexible Self-organisation for the Cloud-Edge Continuum: a Macro-programming Approach**
  - _Authors_: Nicolas Farabegoli, Mirko Viroli, Roberto Casadei
  - _DOI_: [10.1109/acsos61780.2024.00020](https://doi.org/10.1109/acsos61780.2024.00020)
  - _Year_: 2024

- **Scalability through Pulverisation: Declarative deployment reconfiguration at runtime**
  - _Authors_: Nicolas Farabegoli, Danilo Pianini, Roberto Casadei, Mirko Viroli
  - _DOI_: [10.1016/j.future.2024.07.042](https://doi.org/10.1016/j.future.2024.07.042)
  - _Year_: 2024

- **A Demonstrator for Self-organizing Robot Teams**
  - _Authors_: Gianluca Aguzzi, Lorenzo Bacchini, Martina Baiardi, Roberto Casadei, Angela Cortecchia, Davide Domini, Nicolas Farabegoli, Danilo Pianini, Mirko Viroli
  - _DOI_: [10.1007/978-3-031-95589-1_12](https://doi.org/10.1007/978-3-031-95589-1_12)
  - _Year_: 2025

### Dynamic Reconfiguration

- **Heterogeneous GNN for collective-task offloading in cloud-edge via deep Q-learning**
  - _Authors_: Nicolas Farabegoli, Davide Domini, Gianluca Aguzzi, Mirko Viroli
  - _DOI_: [10.1016/j.future.2026.108539](https://doi.org/10.1016/j.future.2026.108539)
  - _Year_: 2026

- **Declarative Deployment Planning for Green Pulverised Collective Computational Systems**
  - _Authors_: Antonio Brogi, Roberto Casadei, Nicolas Farabegoli, Stefano Forti, Mirko Viroli
  - _DOI_: [10.1007/978-3-031-95589-1_6](https://doi.org/10.1007/978-3-031-95589-1_6)
  - _Year_: 2025

- **Dynamic IoT deployment reconfiguration: A global-level self-organisation approach**
  - _Authors_: Nicolas Farabegoli, Danilo Pianini, Roberto Casadei, Mirko Viroli
  - _DOI_: [10.1016/j.iot.2024.101412](https://doi.org/10.1016/j.iot.2024.101412)
  - _Year_: 2024

## Background

As following, the list of papers relevant to the background of the thesis.
They are divided in directly related, i.e., papers directly cited in the papers of the thesis, and indirectly related, i.e., papers that are not directly cited but are relevant to the topic of the thesis.

### Directly Related
- **Ur/Web: A Simple Model for Programming the Web**
  - _Authors_: Chlipala, Adam
  - _Year_: 2015

- **Eliom: A core ML language for Tierless Web Programming**
  - _Authors_: Radanne, Gabriel, Vouillon, Jérôme, Balat, Vincent
  - _Year_: 2016

- **A Higher-Order Calculus of Computational Fields**
  - _Authors_: Audrito, Giorgio, Viroli, Mirko, Damiani, Ferruccio, Pianini, Danilo, Beal, Jacob
  - _Year_: 2019

- **Kickstarting Choreographic Programming**
  - _Authors_: Montesi, Fabrizio
  - _Year_: 2014

- **Language-integrated provenance by trace analysis**
  - _Authors_: Fehrenbach, Stefan, Cheney, James
  - _Year_: 2019

- **Handlers of Algebraic Effects**
  - _Authors_: Gordon D. Plotkin, Matija Pretnar
  - _Year_: 2009

- **Hop: a language for programming the web 2.0**
  - _Authors_: Manuel Serrano, Erick Gallesio, Florian Loitsch
  - _Year_: 2006

- **A Polymorphic RPC Calculus**
  - _Authors_: Kwanghoon Choi, James Cheney, Simon Fowler, Sam Lindley
  - _Year_: 2020

- **Type-Safe Dynamic Placement with First-Class Placed Values**
  - _Authors_: Zakhour, George, Weisenburger, Pascal, Salvaneschi, Guido
  - _Year_: 2023

- **Efficient, Portable, Census-Polymorphic Choreographic Programming**
  - _Authors_: Mako Bates, Shun Kashiwa, Syed Jafri, Gan Shen, Lindsey Kuper, Joseph P. Near
  - _Year_: 2025

- **Degrees of Separation: A Flexible Type System for Safe Concurrency**
  - _Authors_: Yichen Xu, Aleksander Boruch-Gruszecki, Martin Odersky
  - _Year_: 2024

- **Dynamic Choreographies: Theory And Implementation**
  - _Authors_: Preda, Mila Dalla, Gabbrielli, Maurizio, Giallorenzo, Saverio, Lanese, Ivan, Mauro, Jacopo
  - _Year_: 2017

- **Bridging the Gap between Interaction- and Process-Oriented Choreographies**
  - _Authors_: Lanese, Ivan, Guidi, Claudio, Montesi, Fabrizio, Zavattaro, Gianluigi
  - _Year_: 2008

- **Lightweight Functional Session Types**
  - _Authors_: Lindley, Sam, Morris, J. Garrett
  - _Year_: 2017

- **The Coq Proof Assistant, version 8.11.0**
  - _Authors_: Coq Development Team
  - _Year_: 2016

- **Continuation Passing Style for Effect Handlers**
  - _Authors_: Hillerström, Daniel, Lindley, Sam, Atkey, Robert, Sivaramakrishnan, KC
  - _Year_: 2017

- **Tierless Web Programming in the Large**
  - _Authors_: Radanne, Gabriel, Vouillon, Jérôme
  - _Year_: 2018

- **Ur/Web: A Simple Model for Programming the Web**
  - _Authors_: Chlipala, Adam
  - _Year_: 2015

- **ALLIANCE: An architecture for fault tolerant multirobot cooperation**
  - _Authors_: Parker, Lynne E
  - _Year_: 1998

- **Choreographies in Practice**
  - _Authors_: Cruz-Filipe, Luís, Montesi, Fabrizio
  - _Year_: 2016

- **Effects as capabilities: effect handlers and lightweight effect polymorphism**
  - _Authors_: Brachth\"auser, Jonathan Immanuel, Schuster, Philipp, Ostermann, Klaus
  - _Year_: 2020

- **Protelis: practical aggregate programming**
  - _Authors_: Pianini, Danilo, Viroli, Mirko, Beal, Jacob
  - _Year_: Unknown Year

- **Team-level programming of drone sensor networks**
  - _Authors_: Luca Mottola, Mattia Moretta, Kamin Whitehouse, Carlo Ghezzi
  - _Year_: 2014

- **Links: Web Programming without Tiers**
  - _Authors_: Cooper, Ezra, Lindley, Sam, Wadler, Philip, Yallop, Jeremy
  - _Year_: 2006

- **Kleisli**
  - _Authors_: Wong, Limsoon
  - _Year_: 2000

- **Decentralized proximity-aware clustering for collective self-federated learning**
  - _Authors_: Davide Domini, Nicolas Farabegoli, Gianluca Aguzzi, Mirko Viroli, Lukas Esterle
  - _Year_: 2026

- **The Proof Theory and Semantics of Intuitionistic Modal Logic**
  - _Authors_: Simpson, Alex K.
  - _Year_: 1994

- **Choreographies in Practice**
  - _Authors_: Cruz-Filipe, Luís, Montesi, Fabrizio
  - _Year_: 2016

- **From sequential programs to multi-tier applications by program transformation**
  - _Authors_: Neubauer, Matthias, Thiemann, Peter
  - _Year_: 2005

- **Macroprogramming: Concepts, State of the Art, and Opportunities of Macroscopic Behaviour Modelling**
  - _Authors_: Roberto Casadei
  - _Year_: 2023

- **Kilogrid: a novel experimental environment for the Kilobot robot**
  - _Authors_: Valentini, Gabriele, Antoun, Anthony, Trabattoni, Marco, Wiandt, Bern\'at, Tamura, Yasumasa, Hocquard, Etienne, Trianni, Vito, Dorigo, Marco
  - _Year_: 2018

- **Pluralities, Collectives, and Composites**
  - _Authors_: Claudio Masolo, Laure Vieu, Roberta Ferrario, Stefano Borgo, Daniele Porello
  - _Year_: 2020

- **From distributed coordination to field calculus and aggregate computing**
  - _Authors_: Mirko Viroli, Jacob Beal, Ferruccio Damiani, Giorgio Audrito, Roberto Casadei, Danilo Pianini
  - _Year_: 2019

- **Hop**
  - _Authors_: Serrano, Manuel, Gallesio, Erick, Loitsch, Florian
  - _Year_: 2006

- **Ferrite: A Judgmental Embedding of Session Types in Rust**
  - _Authors_: Chen, Ruo Fei, Balzer, Stephanie, Toninho, Bernardo
  - _Year_: 2022

- **Scalability through Pulverisation: Declarative deployment reconfiguration at runtime**
  - _Authors_: Nicolas Farabegoli, Danilo Pianini, Roberto Casadei, Mirko Viroli
  - _Year_: 2024

- **A glimpse of Hopjs**
  - _Authors_: Serrano, Manuel, Prunet, Vincent
  - _Year_: 2016

- **FCPP to aggregate them all**
  - _Authors_: Giorgio Audrito, Gianluca Torta
  - _Year_: 2024

- **Procedural Choreographic Programming**
  - _Authors_: Cruz-Filipe, Luís, Montesi, Fabrizio
  - _Year_: 2017

- **Application partitioning algorithms in mobile cloud computing: Taxonomy, review and future directions**
  - _Authors_: Jie-yao Liu, Ejaz Ahmed, Muhammad Shiraz, Abdullah Gani, Rajkumar Buyya, Ahsan Qureshi
  - _Year_: 2015

- **Deadlock-Freedom-by-Design: Multiparty Asynchronous Global Programming**
  - _Authors_: Carbone, Marco, Montesi, Fabrizio
  - _Year_: 2013

- **Flexible Self-organisation for the Cloud-Edge Continuum: a Macro-programming Approach**
  - _Authors_: Nicolas Farabegoli, Mirko Viroli, Roberto Casadei
  - _Year_: 2024

- **Discretionary Information Flow Control for Interaction-Oriented Specifications**
  - _Authors_: Lluch Lafuente, Alberto, Nielson, Flemming, Nielson, Hanne Riis
  - _Year_: 2015

- **Effekt: Capability-Passing Style for Effect Safety**
  - _Authors_: Jonathan Immanuel Brachth\"auser, Philipp Schuster, Klaus Ostermann
  - _Year_: 2021

- **micros_swarm_framework**
  - _Authors_: ROS Wiki --- wiki.ros.org -, title = micros\_swarm\_framework - ROS Wiki --- wiki.ros.org
  - _Year_: Unknown Year

- **PySwarming: a research toolkit for Swarm Robotics**
  - _Authors_: Emerson Martins de Andrade, Antonio Carlos Fernandes, Joel Sena Sales
  - _Year_: 2023

- **ChoRus: Library-Level Choreographic Programming in Rust**
  - _Authors_: Kashiwa, Shun, Kuper, Lindsey
  - _Year_: 2024

- **Automatic deployment of distributed software systems: Definitions and state of the art**
  - _Authors_: Jean-Paul Arcangeli, Raja Boujbel, S\'ebastien Leriche
  - _Year_: 2015

- **Type-safe distributed programming with ML5**
  - _Authors_: Murphy, Tom, Crary, Karl, Harper, Robert
  - _Year_: 2007

- **Capturing Types**
  - _Authors_: Aleksander Boruch-Gruszecki, Martin Odersky, Edward Lee, Ondrej Lhot\'ak, Jonathan Immanuel Brachth\"auser
  - _Year_: 2023

- **Polymorphic Effect Systems**
  - _Authors_: John M. Lucassen, David K. Gifford
  - _Year_: 1988

- **Web Services Choreography Description Language Version 1.0**
  - _Authors_: W3C WS-CDL Working Group
  - _Year_: 2005

- **Query Lifting: Language-integrated query for heterogeneous nested collections**
  - _Authors_: Ricciotti, Wilmer, Cheney, James
  - _Year_: 2021

- **An Actor-based Programming Framework for Swarm Robotic Systems**
  - _Authors_: Wei Yi, Bin Di, Ruihao Li, Huadong Dai, Xiaodong Yi, Yanzhen Wang, Xuejun Yang
  - _Year_: 2020

- **Towards reinforcement learning-based aggregate computing**
  - _Authors_: Aguzzi, Gianluca, Casadei, Roberto, Viroli, Mirko
  - _Year_: 2022

- **Dynamic IoT deployment reconfiguration: A global-level self-organisation approach**
  - _Authors_: Nicolas Farabegoli, Danilo Pianini, Roberto Casadei, Mirko Viroli
  - _Year_: 2024

- **Distributed system development with ScalaLoci**
  - _Authors_: Pascal Weisenburger, Mirko K\"ohler, Guido Salvaneschi
  - _Year_: 2018

- **Search-based Tier Assignment for Optimising Offline Availability in Multi-tier Web Applications**
  - _Authors_: Philips, Laure, De Koster, Joeri, De Meuter, Wolfgang, De Roover, Coen
  - _Year_: 2018

- **Robust Composition: Towards a Unified Approach to Access Control and Concurrency Control**
  - _Authors_: Mark S. Miller
  - _Year_: 2006

- **Exceptional asynchronous session types: session types without tiers**
  - _Authors_: Fowler, Simon, Lindley, Sam, Morris, J. Garrett, Decova, Sára
  - _Year_: 2019

- **Scafi: A scala DSL and toolkit for aggregate programming**
  - _Authors_: Casadei, Roberto, Viroli, Mirko, Aguzzi, Gianluca, Pianini, Danilo
  - _Year_: 2022

- **A Classification Framework for Software Component Models**
  - _Authors_: Ivica Crnkovic, S\'everine Sentilles, Aneta Vulgarakis, Michel R. V. Chaudron
  - _Year_: 2011

- **Service-Oriented Programming with Jolie**
  - _Authors_: Fabrizio Montesi, Claudio Guidi, Gianluigi Zavattaro
  - _Year_: 2014

- **Declarative Macro-Programming of Collective Systems with Aggregate Computing: An Experience Report**
  - _Authors_: Casadei, Roberto, Viroli, Mirko
  - _Year_: 2024

- **A Symmetric Modal Lambda Calculus for Distributed Computing**
  - _Authors_: Murphy VII, Tom, Crary, Karl, Harper, Robert, Pfenning, Frank
  - _Year_: 2004

- **Towards Lightweight and Efficient Choreographic Cloud Services**
  - _Authors_: Alex Ionescu, Alejandro Russo
  - _Year_: 2026

- **Links: Web Programming Without Tiers**
  - _Authors_: Ezra Cooper, Sam Lindley, Philip Wadler, Jeremy Yallop
  - _Year_: 2006

- **Buzz: A Programming Language for Robot Swarms**
  - _Authors_: Carlo Pinciroli, Giovanni Beltrame
  - _Year_: 2016

- **Choral: Object-oriented Choreographic Programming**
  - _Authors_: Giallorenzo, Saverio, Montesi, Fabrizio, Peressotti, Marco
  - _Year_: 2024

- **A Survey of Multitier Programming**
  - _Authors_: Pascal Weisenburger, Johannes Wirth, Guido Salvaneschi
  - _Year_: 2021

- **ARGoS: a modular, parallel, multi-engine simulator for multi-robot systems**
  - _Authors_: Pinciroli, Carlo, Trianni, Vito, O’Grady, Rehan, Pini, Giovanni, Brutschy, Arne, Brambilla, Manuele, Mathews, Nithin, Ferrante, Eliseo, Di Caro, Gianni, Ducatelle, Frederick, others
  - _Year_: 2012

- **Ocsigen: Typing Web Interaction with Objective Caml**
  - _Authors_: Balat, Vincent
  - _Year_: 2006

- **Alice or Bob?: Process polymorphism in choreographies**
  - _Authors_: Graversen, Eva, Hirsch, Andrew K., Montesi, Fabrizio
  - _Year_: Unknown Year

- **Scala 3 Reference**
  - _Authors_: Martin Odersky et al.
  - _Year_: 2021

- **A Methodology and Simulation-Based Toolchain for Estimating Deployment Performance of Smart Collective Services at the Edge**
  - _Authors_: Roberto Casadei, Giancarlo Fortino, Danilo Pianini, Andrea Placuzzi, Claudio Savaglio, Mirko Viroli
  - _Year_: 2022

- **Engineering collective intelligence at the edge with aggregate processes**
  - _Authors_: Roberto Casadei, Mirko Viroli, Giorgio Audrito, Danilo Pianini, Ferruccio Damiani
  - _Year_: 2021

- **Scribbling Interactions with a Formal Foundation**
  - _Authors_: Kohei Honda, Aybek Mukhamedov, Gary Brown, Tzu-Chun Chen, Nobuko Yoshida
  - _Year_: 2011

- **Introduction to Choreographies**
  - _Authors_: Montesi, Fabrizio
  - _Year_: 2023

- **The eXchange Calculus (XC): A functional programming language design for distributed collective systems**
  - _Authors_: Giorgio Audrito, Roberto Casadei, Ferruccio Damiani, Guido Salvaneschi, Mirko Viroli
  - _Year_: 2024

- **Querying in C#: How Language Integrated Query (LINQ) Works**
  - _Authors_: Torgersen, Mads
  - _Year_: 2007

- **HasChor: Functional Choreographic Programming for All (Functional Pearl)**
  - _Authors_: Shen, Gan, Kashiwa, Shun, Kuper, Lindsey
  - _Year_: 2023

- **Algebraic Operations and Generic Effects**
  - _Authors_: Gordon D. Plotkin, John Power
  - _Year_: Unknown Year

- **Pirouette: Higher-Order Typed Functional Choreographies**
  - _Authors_: Hirsch, Andrew K., Garg, Deepak
  - _Year_: 2022

- **Rigorous engineering of collective adaptive systems: special section**
  - _Authors_: Rocco De Nicola, Stefan J\"ahnichen, Martin Wirsing
  - _Year_: 2020

- **Multitier Modules**
  - _Authors_: Weisenburger, Pascal, Salvaneschi, Guido
  - _Year_: 2019

- **Meld: A declarative approach to programming ensembles**
  - _Authors_: Michael P. Ashley-Rollman, Seth Copen Goldstein, Peter Lee, Todd C. Mowry, Padmanabhan Pillai
  - _Year_: 2007

- **The DReAM framework for dynamic reconfigurable architecture modelling: theory and applications**
  - _Authors_: Rocco De Nicola, Alessandro Maggi, Joseph Sifakis
  - _Year_: 2020

- **Swarm Robotics: A Perspective on the Latest Reviewed Concepts and Applications**
  - _Authors_: Pollyanna G. Faria Dias, Mateus C. Silva, Geraldo P. Rocha Filho, Patrícia A. Vargas, Luciano Perdigão Cota, Gustavo Pessin
  - _Year_: 2021

- **A Methodology and Simulation-Based Toolchain for Estimating Deployment Performance of Smart Collective Services at the Edge**
  - _Authors_: Roberto Casadei, Giancarlo Fortino, Danilo Pianini, Andrea Placuzzi, Claudio Savaglio, Mirko Viroli
  - _Year_: 2022

- **Deadlock-freedom-by-design: multiparty asynchronous global programming**
  - _Authors_: Carbone, Marco, Montesi, Fabrizio
  - _Year_: 2013

- **Language-Integrated Query for Temporal Data**
  - _Authors_: Fowler, Simon, Galpin, Vashti, Cheney, James
  - _Year_: 2022

- **Functional Choreographic Programming**
  - _Authors_: Cruz-Filipe, Luís, Graversen, Eva, Lugović, Lovro, Montesi, Fabrizio, Peressotti, Marco
  - _Year_: 2022

- **Cyberbotics Ltd. Webots™: Professional Mobile Robot Simulation**
  - _Authors_: Michel, Olivier
  - _Year_: 2004

- **Dynamic choreographies: Theory and implementation**
  - _Authors_: Dalla Preda, Mila, Gabbrielli, Maurizio, Giallorenzo, Saverio, Lanese, Ivan, Mauro, Jacopo
  - _Year_: 2017

- **A Brief History of Process Algebra**
  - _Authors_: Baeten, J. C. M.
  - _Year_: 2005

- **Time-Fluid Field-Based Coordination through Programmable Distributed Schedulers**
  - _Authors_: Danilo Pianini, Roberto Casadei, Mirko Viroli, Stefano Mariani, Franco Zambonelli
  - _Year_: 2021

