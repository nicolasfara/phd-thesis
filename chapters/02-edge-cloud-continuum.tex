...
the computing infrastructure stretching between cloud data centres and the devices embedded in the physical world.
%
This chapter opens Part I by fixing that substrate.
%
It establishes what the edge-cloud continuum is,
which environments this thesis must serve on it -- \ac{IoT} ecosystems and robot swarms --
and why the architectural paradigms currently used to structure distributed software cannot deploy applications on it at the granularity,
or with the runtime flexibility,
those environments demand.
%
The gap this review exposes is the one the pulverisation model of \cref{chap:pulverization} closes;
the other two background chapters ask, against the same substrate,
how collective systems can be programmed (\cref{chap:programming-models}) and how their placement can be revised while they run (\cref{chap:dynamic-reconfiguration}).
...
%
\acp{CPS} and robot swarms are natural stress tests for the continuum characterised in \cref{sec:the-continuum}:
...
frames them as instances of a broader class introduced in \cref{subsec:collective-adaptive-systems},
...
%
\Cref{chap:demonstrator} returns to this setting at the end of the thesis,
exercising the models and platform this thesis develops end to end on a self-organising robot team.
...
The SCEL language responds to exactly this requirement at the language level~\cite{DBLP:journals/taas/NicolaLPT14}.
%
...
making ensemble formation self-organising in the sense fixed in \cref{subsec:swarm-robotics}.
%
...
%
Introduced by Casadei et al.~\cite{DBLP:journals/fi/CasadeiPPVW20},
it splits each logical device of a system into a small set of independently deployable components -- sensing,
actuation,
behaviour,
state,
and communication --
...
%
The mapping from components to hosts can then be revised at runtime,
declaratively and without touching the application logic~\cite{DBLP:journals/fgcs/FarabegoliPCV24}.
%
This thesis builds upon that direction and develops it in full in \cref{chap:pulverization},
