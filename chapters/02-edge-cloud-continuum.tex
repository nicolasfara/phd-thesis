...
%
This chapter fixes that substrate:
what the edge-cloud continuum is,
...
%
The resulting gap is the one the pulverisation model closes (\cref{chap:pulverization});
...
%
\ac{IoT} ecosystems and swarm robotics (\cref{subsec:iot-systems,subsec:swarm-robotics}) are read as applications of a broader class (\cref{subsec:collective-adaptive-systems}),
and share the deployment and coordination challenges drawn out at the end of the section (\cref{subsec:environment-challenges}).
...
running one macro-program on a physical robot team to show that a deployment the thesis's model admits can be built on real hardware.
...
Microservices,
\ac{FaaS},
and the actor model each adapted this progression for edge deployment (\cref{subsec:microservices-faas-edge,subsec:actor-model-edge}),
