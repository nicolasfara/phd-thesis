...
%
The literature offers four answers to that need:
...
energy is one such objective,
already flagged in \cref{subsec:continuum-characteristics} as a concern orthogonal to raw computational capacity and treated as first-class in \cref{subsec:energy-aware-orchestration}.
%
...
in the specification models reviewed in \cref{subsec:declarative-deployment}.
%
...
a classification followed loosely here,
with one addition:
the first two categories fall under constrained optimisation,
with energy treated as an objective of its own (\cref{subsec:constraint-based-placement,subsec:energy-aware-orchestration});
the addition is how the problem itself is stated, rather than how it is solved,
a concern the classification leaves implicit (\cref{subsec:declarative-deployment});
and the third category, learning-based methods,
is what both surveys register as the most active recent trend (\cref{sec:ml-for-topologies,fig:orchestration-taxonomy}).
...
the contrast between the learned approach and the heuristics of \cref{sec:edge-orchestration-sota} on task offloading, where the decision narrows to a single unit and a binary choice of host so the contrast with a re-solved optimisation stands out most sharply (\cref{subsec:learning-based-offloading});
...
the combination later developed into a full offloading policy (\cref{chap:heterogeneous-gnn}).
