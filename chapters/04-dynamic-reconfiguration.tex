...
Reopening the mapping at runtime raises four questions that structure the rest of this chapter.
%
...
%
Third, it must decide \emph{in what form} the problem itself is stated:
how the components, their requirements, and the objective are declared to whatever computes the mapping.
\Cref{subsec:declarative-deployment} reviews the specification models that carry this statement.
%
...
\cref{subsec:declarative-deployment} steps back from how a placement is computed to how the problem is stated -- a concern the classification leaves implicit --
...
%
How a planner is told what a green deployment is -- how the components, the infrastructure, and objectives such as these are stated for it -- is a question of its own, taken up next.
...
%
The declared state, however, remains an operations artefact: it belongs to whoever runs the platform, not to the application -- the same boundary \cref{subsec:constraint-based-placement} found around the placement policy itself.
...
%
For collective systems, the declarative statement need not stop at operations: it can become the artefact the application itself manipulates at runtime, the direction taken for pulverised deployments~\cite{DBLP:journals/fgcs/FarabegoliPCV24}.

Energy and carbon objectives tracked against a time-varying supply (\cref{subsec:energy-aware-orchestration}) and a deployment goal stated once and kept satisfied are exactly the two threads \cref{chap:deployment-and-reconfiguration} combines into a declarative planner for green deployments on the continuum.
%
Treating placement as something learned online, rather than declared and repaired, is the shift that carries the chapter into the learning-based methods examined in \cref{sec:ml-for-topologies}.
