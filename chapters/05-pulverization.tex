...
Pulverisation,
due to Casadei et al. and already recalled in \cref{chap:edge-cloud-continuum}~\cite{DBLP:journals/fi/CasadeiPPVW20},
decouples a collective system's self-organising logic from its deployment by splitting each logical device into a small,
...
state,
and communication;
this thesis develops the idea in full here.
%
Aggregate computing,
choreographic programming,
and multitier programming reach the same gap from the language side (\cref{chap:programming-models}):
each derive logical units of computation from a global specification of collective behaviour but leave the mapping of those units onto a changing physical topology either implicit or fixed at compile time,
and aggregate computing's own execution model further assumes that every device runs a complete instance of the aggregate program,
not a fragment of one realised across heterogeneous target platforms.
%
That mapping cannot even be posed as a runtime decision without a model of what the unit being mapped actually is (\cref{chap:dynamic-reconfiguration}).

This chapter supplies that model.
%
The rest of the chapter first states why the original,
fixed-role pulverisation and its declarative,
runtime-reconfigurable extension are not enough on their own and sets out the more general,
orthogonal partitioning developed here instead (\cref{sec:pulverization-motivation}),
then defines that partitioning's core abstractions:
application and infrastructural devices,
the macro-program as a directed acyclic graph of deployable components,
the local/collective distinction,
and the forwarding chains that map components onto physical devices (\cref{sec:pulverization-abstractions}).
%
It then fixes the model's execution semantics,
...
and confirms it in simulation (\cref{sec:pulverization-semantics}),
before closing by relating the model to the pulverisation it generalises and stating what it deliberately leaves open (\cref{sec:pulverization-discussion}).
...
%
The first two goals are met by the partitioning developed next (\cref{sec:pulverization-abstractions});
the third is proved once its execution semantics are in place (\cref{sec:pulverization-semantics}).
%
...
%
The execution model fixes how rounds and messages proceed (\cref{subsec:pulverization-execution-model});
the guarantee that follows is stated and proved (\cref{subsec:deployment-independence})
and confirmed in simulation on two collective services (\cref{subsec:pulverization-validation}).
...
%
One such given deployment is built on physical robots in \cref{chap:demonstrator}, where the choice is made once, by hand, before the system starts.
