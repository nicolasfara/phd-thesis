...
The deployment is not fixed at release time.
%
...
connected in an arbitrary graph.
No component is assigned one of five fixed slots.
%
...
The four subsections that follow introduce the model's vocabulary in the order the concepts build on one another:
...
Each role reflects the part a device plays in a given deployment, not any physical property of the device itself.
%
...
This is why \cref{subsec:deployment-independence} proves its guarantee: the guarantee cannot simply be assumed.
...
There, a logical device always decomposed into exactly five components.
Here, the number of components is whatever the macro-program's own data-flow graph calls for,
from one to arbitrarily many,
and the number of instances per device that a deployment must place follows the same count.
...
This is the same neighbour-dependent computation that gives aggregate computing's fields their collective character (\cref{subsec:aggregate-computing}),
but here it is scoped to a single component of a larger macro-program, not to the whole macro-program at once.
%
...
Collectiveness is a property of individual components, not of a logical device as a whole.
A single macro-program can therefore freely mix components that behave like self-organising, field-based services with components that behave like ordinary, non-coordinating ones,
...
The first concerns timing: the assignment is a per-component, per-device decision taken -- and revisable -- at runtime, not fixed once at compile time.
The second concerns what drives the decision: forwarding follows whether a host's exposed capabilities meet a component's requirements,
in the sense PulvReAKt already established for the fixed five-role case (\cref{sec:pulverization-motivation}),
not a static tier annotation written by the programmer.
...
The two cuts are orthogonal, not competing:
