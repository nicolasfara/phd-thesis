...
%
Both paradigms provide point-to-point exchange and little else;
...
and nothing checks whether the peer reading it is allowed to.
%
...
%
There is no third case where two paradigms interact directly and neither one's rules apply.
...
plus a \code{handle} construct that supplies a capability and so starts execution.
%
...
The first question is expressivity: whether the programs that three dedicated languages support can be written in one capability-based framework.
%
...
%
They fit in about 500 lines, with five points where one paradigm reads a value produced by another.
...
while the reimplementation needs seven communication terms.
%
...
a paradigm's operations are available where its capability is in scope, and placement types carry values across the boundaries.
%
...
peer types, and ties between them, each with a cardinality.
%
...
so the program needs no separate barrier.
%
...
%
It works for any number of backups, while the choreographic implementation needs the replica count fixed statically~\cite{Farabegoli:2026:ScalaTropy}.
...
and naming the pattern in the operation makes the intent visible, instead of leaving the reader to recover it from the type.
...
Both designs refuse programs that the paradigms they unify would accept.
%
\camil rejects nested placed scopes, which existing multitier languages allow.
%
The rule is coarse: it rejects every nesting, not only the ones that deadlock, and that is the price of ruling deadlock out.
%
